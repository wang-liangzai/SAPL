...
class multiTRAR_SA_block(nn.Module):
    ...
    def forward(self, x, y, y_masks, x_mask, tau, training):  # x (64, 49, 512) I T y
...
def getMasks_img_multimodal(x_mask, __C):
    ...
    for order in ORDERS:
        if order == 0:
            mask_list.append(x_mask)  # 全不掩码，也就是能看到全局信息
        else:
            mask_img = torch.from_numpy(getImgMasks(__C["IMG_SCALE"], order)).byte().to(x_mask.device)  # (49, 49) -》256*256
            mask = torch.concat([mask_img]*(__C["len"]//(__C["IMG_SCALE"]*__C["IMG_SCALE"])), dim=0)  # 98*49
            mask = torch.concat([mask, mask_img[:(__C["len"] % (__C["IMG_SCALE"]*__C["IMG_SCALE"])), :]])  # 98*49 + 2*49
...
class DynRT_ED(nn.Module):
    ...
    def forward(self, y, x, y_mask, x_mask):
        # y text (bs, max_len, dim) x img (bs, gird_num, dim) y_mask (bs, 1, 1, max_len) x_mask (bs, 1, 1, grid_num) x:image,y:text
        ...
        for i, dec in enumerate(self.dec_list):
            y = dec(y, x, x_masks[:i+1], y_mask, self.tau, self.training)  # (4, 360, 768) I T
        return y, x
